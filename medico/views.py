...
# Regras de negócios e validações de consultas_medico
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        if consulta.status == 'C':
            messages.add_message(request, constants.WARNING, 'Esta consulta já foi cancelada')
            return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
        elif consulta.status == 'F':
            messages.add_message(request, constants.WARNING, 'Esta consulta já foi finalizada')
            return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
...
def finalizar_consulta(request, id_consulta):
    ...
    consulta = Consulta.objects.get(id=id_consulta)
    #validação se a consulta é do mesmo user
...
# Regras de negócios e validações para upload de documento
def add_documento(request, id_consulta):
    ...
    if not documento:
        messages.add_message(request, constants.ERROR, 'Anexe um documento')
        return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
